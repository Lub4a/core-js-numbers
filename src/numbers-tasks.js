...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const difference1 = x2 - x1;
  const difference2 = y2 - y1;
  const power1 = difference1 * difference1;
  const power2 = difference2 * difference2;
...
function getLastDigit(value) {
  ...
  const againNumber = Number(lastNumber);
  return againNumber;
}
...
function numberToStringInBase(number, base) {
  let result = 0;
  const p = Number.parseInt(number, base);
  if (Number.isNaN(p)) {
    result = 0;
  } else {
    return p;
